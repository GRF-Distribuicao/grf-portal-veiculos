...
import { Building2, Check, Clock3, MailCheck, MailWarning, Truck, UserCheck, UserX } from "lucide-react";
...
export function TransporterAccessRequests() {
  ...
  async function handleReject(row: TransporterRequest) {
    ...
    setBusyId(row.id);
    const result = await reject({ data: { requestId: row.id, companyId: null, createNewCompany: false, note: "" } });
    setBusyId(null);
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-blue-600">
            ...
            <span className="text-xs font-bold tracking-wide uppercase">Acessos de transportadoras</span>
          </div>
          <h2 className="mt-2 font-display text-2xl font-extrabold">Área do Transportador</h2>
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
            Confirme a empresa correta antes de liberar. O vínculo aprovado define exatamente quais veículos esse usuário poderá visualizar.
          </p>
        </div>
        ...
        <>
          ...
          {pending.length === 0 ? (
            ...
            <div className="mt-4 grid gap-4">
              {pending.map((row) => {
                const selected = companySelection[row.id] ?? "";
                const suggested = row.suggested_company_id ? companyById.get(row.suggested_company_id) : null;
                return (
                  <article key={row.id} className="rounded-xl border border-blue-500/20 bg-card p-5 shadow-sm">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm">
                          <span><strong>Empresa:</strong> {row.company_name}</span>
                          <span><strong>CNPJ:</strong> {formatCnpj(row.company_cnpj)}</span>
                        </div>
                        <p className="mt-2 text-xs text-muted-foreground">Solicitado em {formatDateTime(row.requested_at)}</p>
                      </div>
                    ...
                    <div className="mt-4 flex items-center gap-2 text-xs">
                      {row.notification_sent_at ? (
                        <span className="inline-flex items-center gap-1.5 text-success"><MailCheck className="size-4" /> Responsáveis notificados por e-mail</span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 text-muted-foreground" title={row.notification_error ?? undefined}><MailWarning className="size-4" /> Solicitação registrada no controle de acesso</span>
                      )}
                    </div>
                    ...
                    <div className="mt-5 rounded-lg border border-border bg-surface/60 p-4">
                      ...
                      {suggested && (
                        <p className="mt-1 text-xs text-muted-foreground">
                          O sistema encontrou uma possível correspondência: <strong>{suggested.name}</strong> · {suggested.vehicleCount} veículo(s).
                        </p>
                      )}
                      {!suggested && (
                        <p className="mt-1 text-xs text-muted-foreground">
                          Nenhuma correspondência exata foi encontrada. Selecione uma empresa existente ou crie uma nova somente após conferir os dados.
                        </p>
                      )}

                      <div className="mt-4 max-w-xl">
                        <Label className="text-xs font-semibold">Transportadora confirmada pela GRF</Label>
                        <Select
                          value={selected}
                          onValueChange={(value) => setCompanySelection((current) => ({ ...current, [row.id]: value }))}
                        >
                          <SelectTrigger className="mt-1.5 w-full"><SelectValue placeholder="Selecione a empresa correta" /></SelectTrigger>
                          <SelectContent>
                            {companies.map((company) => (
                              <SelectItem key={company.id} value={company.id}>
                                {company.name} · {company.vehicleCount} veículo(s){company.cnpj ? ` · ${formatCnpj(company.cnpj)}` : ""}
                              </SelectItem>
                            ))}
                            <SelectItem value="NEW">+ Criar nova transportadora com os dados solicitados</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 flex flex-wrap gap-3 border-t border-border pt-4">
                      <Button className="bg-blue-600 text-white hover:bg-blue-700" disabled={busyId === row.id} onClick={() => void handleApprove(row)}>
                        <UserCheck className="size-4" /> Aprovar e vincular
                      </Button>
                      <Button variant="outline" disabled={busyId === row.id} onClick={() => void handleReject(row)}>
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
            {history.length === 0 ? (
              <p className="p-6 text-sm text-muted-foreground">Ainda não há decisões de transportadoras registradas.</p>
            ) : (
              <div className="divide-y divide-border">
                {history.slice(0, 30).map((row) => {
                  const company = row.matched_company_id ? companyById.get(row.matched_company_id) : null;
                  return (
                    <div key={row.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">{row.email} · {row.company_name} · {formatCnpj(row.company_cnpj)}</p>
                        {company && <p className="mt-1 text-xs font-semibold text-blue-700">Vinculado a: {company.name}</p>}
                      </div>
                      <div className="text-right">
                        <span className={row.status === "APPROVED" ? "inline-flex rounded-full border border-success/30 bg-success/10 px-2.5 py-1 text-xs font-bold text-success" : "inline-flex rounded-full border border-destructive/30 bg-destructive/10 px-2.5 py-1 text-xs font-bold text-destructive"}>
                          {row.status === "APPROVED" ? "Aprovado" : "Reprovado"}
                        </span>
                        {row.decided_at && <p className="mt-1 text-xs text-muted-foreground">{formatDateTime(row.decided_at)}</p>}
                      </div>
                    </div>
                  );
                })}
